...
import { participanteSelect, mapParejaToParticipante } from './participanteService.js'
...
export const matchSelect = 'id,categoria_id,ronda,fase,grupo_id,orden,pista_id,pareja_a_id,pareja_b_id,ganador_id,fecha,hora,pista,estado,set1_a,set1_b,set2_a,set2_b,set3_a,set3_b,goles_a,goles_b,penaltis_a,penaltis_b,ganador_manual_id,observaciones,created_at'
...
export async function fetchGruposByCategoria(categoriaId) {
  ...
  const [gruposResult, asignacionesResult, clasificacionesResult, partidosResult] = await Promise.all([
    ...
    supabase.from('partidos').select(matchSelect).eq('categoria_id', categoriaId).eq('fase', 'grupos').order('orden', { ascending: true }),
  ])
...
export async function assignParejaToGrupo({ categoriaId, grupoId, parejaId, orden = 0 }) {
  ensureSupabase()
  requireCategoria(categoriaId)
  requireGrupo(grupoId)
  requirePareja(parejaId)

  const [grupoResult, parejaResult, existingResult] = await Promise.all([
    supabase.from('grupos').select('id,categoria_id,categorias(torneo_id)').eq('id', grupoId).single(),
    supabase.from('parejas').select('id,categoria_id,categorias(torneo_id)').eq('id', parejaId).single(),
    supabase
      .from('grupo_parejas')
  ...
  const grupoTorneoId = Array.isArray(grupoResult.data.categorias)
    ? grupoResult.data.categorias[0]?.torneo_id
    : grupoResult.data.categorias?.torneo_id
  const participanteTorneoId = Array.isArray(parejaResult.data.categorias)
    ? parejaResult.data.categorias[0]?.torneo_id
    : parejaResult.data.categorias?.torneo_id

  if (grupoTorneoId && participanteTorneoId && grupoTorneoId !== participanteTorneoId) {
    throw new Error('El participante seleccionado no pertenece al torneo del grupo.')
  }
...
export async function recalcularClasificacion(categoriaId) {
  ...
  const categoriaResult = await supabase
    .from('categorias')
    .select('criterios_clasificacion,torneos(tipo_deporte)')
    .eq('id', categoriaId)
    .single()
  if (categoriaResult.error) throw categoriaResult.error

  const torneo = Array.isArray(categoriaResult.data?.torneos) ? categoriaResult.data.torneos[0] : categoriaResult.data?.torneos
  const tipoDeporte = torneo?.tipo_deporte || 'padel'
  const criteriosOrden = tipoDeporte === 'futbol_sala'
    ? ['puntos', 'diferencia_goles', 'goles_favor', 'nombre']
    : categoriaResult.data.criterios_clasificacion?.length
  ...
  const parejaIds = [...new Set(grupoParejas.map((asignacion) => asignacion.pareja_id))]
  const participantesResult = parejaIds.length
    ? await supabase.from('parejas').select(participanteSelect).in('id', parejaIds)
    : { data: [], error: null }
  if (participantesResult.error) throw participantesResult.error
  const participantMap = new Map(participantesResult.data.map((pareja) => {
    const participante = mapParejaToParticipante(pareja)
    return [pareja.id, participante]
  }))
  const standings = []
  ...
  grupos.forEach((grupo) => {
    ...
    standings.push(...(tipoDeporte === 'padel'
      ? sorted.sort((a, b) => compareStatsByCriteria(a, b, criteriosOrden, partidosDelGrupo)).map((stats, index) => ({ ...stats, posicion: index + 1 }))
      : sorted))
  })
...
export async function obtenerClasificacion(categoriaId) {
  const { clasificacionesGrupo } = await fetchGruposByCategoria(categoriaId)
  return clasificacionesGrupo
}
...
export async function generateEliminationFromClassified(categoriaId) {
  ...
  if (classifiedPairs.length < 2) {
    throw new Error('No hay suficientes parejas clasificadas para generar cuadro.')
  }
